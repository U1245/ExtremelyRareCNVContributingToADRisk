...
list_of_genes.analysis_PC <- function(type.CNV, filtre.freq.value, filtre.freq.type.recouvrement, list_of_genes, p.value_threshold) {
  ...
  transcript.list.in.matrix <- as.character(subset(filtreCNV, exclusion == FALSE)$transcript.name)
  transcript.list.in.matrix.before.freq.filtre <- filtreCNV$transcript.name
  ...
  if (list_of_genes == "All") {
    ...
    ensemble.list.Gene <- list()
    ensemble.list.Gene[[1]] <- GeneTranscript$Gene
  ...
  # for(iterB in 1:B) {
    ...
    transcript.DMexcluded.ordinal.regression <- as.character(subset(differentialMissingness, Selection == "none" & (is.na(DM.for.ordinal.regression.p.value..chi2.) | DM.for.ordinal.regression.p.value..chi2. < p.value_threshold))$transcript.name)
    ...
    if (length(list.geneSCORE) == 0) { # if none gene correspond to transcript passing filters
      ...
      res1 <- c(rep(NA, 12),  length(ensemble.list.Gene[[iterB]]), count.genes.original.matrix, count.genes.analysable.before.DM.exclusion,
                count.genes.analysable.after.DMexclusion, count.genes.in.list.with1, rep(NA, 20))
      ...
      colnames(res) <- c("list.of.gene.name", "Adjustment", "N CTRL", "N LOAD", "N EOAD", "N total", "Ncarriers CTRL", "Ncarriers LOAD", "Ncarriers EOAD", "Ncarriers total", "Rate CTRL", "Rate LOAD", "Rate EOAD", "Rate total", "N genes in the list", "N genes with related transcript in the matrix", "N genes with related analysable transcript in the matrix", "N genes with related transcript in the matrix after filter for differential missingness", "N genes encompassing at least one CNV among analysable transcripts", "N analysable transcripts encompassing at least one CNV", "min of N genes affected per sample", "max of N genes affected per sample", "min of N genes being NA per sample", "max of N genes being NA per sample", "OR EOAD vs CTRL", "OR EOAD vs CTRL - CIinf", "OR EOAD vs CTRL - CIsup", "OR LOAD vs CTRL", "OR LOAD vs CTRL - CIinf", "OR LOAD vs CTRL - CIsup", "OR clm", "OR clm - CIinf", "OR clm - CIsup", "OR polr", "OR polr - CIinf", "OR polr - CIsup", "p-value (ordinal regression, using clm)", "p-value (ordinal regression, using polr)", "warning clm")
      ...
      dat <- cbind(genematrix[, c("sample", "Status", paste0("PC", 1:10))], sum.listGene)
      ...
      if (sum(dat.EOAD$status.listGene == 1, na.rm = TRUE) == 0) {
        ...
        m <- glm(Y ~ status.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dat.EOAD, family = binomial(link = "logit"))
      ...
      if (sum(dat.LOAD$status.listGene == 1, na.rm = TRUE) == 0) {
        ...
        m <- glm(Y ~ status.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dat.LOAD, family = binomial(link = "logit"))
        OR.LOAD <- c(exp(m$coef)["status.listGene1"], exp(confint(m))["status.listGene1",])
      }
      
      # Ordinal regression with clm
      rego <- clm(Status.f ~ status.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = na.omit(dat))
      ...
      rego2 <- polr(Status.f ~ status.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = na.omit(dat), Hess = TRUE)
      ...
      res <- c(count.ordinal.regression, OR.EOAD, OR.LOAD, OR.clm, OR.polr, pvalue.clm, pvalue.polr, message.clm)
      ...
      colnames(res.binary.withoutAdjustment) <- c("list.of.gene.name", "Adjustment", "N CTRL", "N LOAD", "N EOAD", "N total", "Ncarriers CTRL", "Ncarriers LOAD", "Ncarriers EOAD", "Ncarriers total", "Rate CTRL", "Rate LOAD", "Rate EOAD", "Rate total", "N genes in the list", "N genes with related transcript in the matrix", "N genes with related analysable transcript in the matrix", "N genes with related transcript in the matrix after filter for differential missingness", "N genes encompassing at least one CNV among analysable transcripts", "N analysable transcripts encompassing at least one CNV", "min of N genes affected per sample", "max of N genes affected per sample", "min of N genes being NA per sample", "max of N genes being NA per sample", "OR EOAD vs CTRL", "OR EOAD vs CTRL - CIinf", "OR EOAD vs CTRL - CIsup", "OR LOAD vs CTRL", "OR LOAD vs CTRL - CIinf", "OR LOAD vs CTRL - CIsup", "OR clm", "OR clm - CIinf", "OR clm - CIsup", "OR polr", "OR polr - CIinf", "OR polr - CIsup", "p-value (ordinal regression, using clm)", "p-value (ordinal regression, using polr)", "warning clm")
      ...
      if (sum(dat.EOAD$sum.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.EOAD$Y <- ifelse(dat.EOAD$Status.f == "EOAD", 1, 0)
        m <- glm(Y ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dat.EOAD, family = binomial(link = "logit"))
      ...
      if (sum(dat.LOAD$sum.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.LOAD$Y <- ifelse(dat.LOAD$Status.f == "LOAD", 1, 0)
        m <- glm(Y ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = dat.LOAD, family = binomial(link = "logit"))
        OR.LOAD <- c(exp(m$coef)["sum.listGene"], exp(confint(m))["sum.listGene",])
      }
      
      # Ordinal regression with clm
      rego <- clm(Status.f ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = na.omit(dat))
      ...
      # Ordinal regression with polr
      rego2 <- polr(Status.f ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data = na.omit(dat), Hess = TRUE)
      ...
      colnames(res.sum.withoutAdjustment) <- c("list.of.gene.name", "Adjustment", "N CTRL", "N LOAD", "N EOAD", "N total", "Ncarriers CTRL", "Ncarriers LOAD", "Ncarriers EOAD", "Ncarriers total", "Rate CTRL", "Rate LOAD", "Rate EOAD", "Rate total", "N genes in the list", "N genes with related transcript in the matrix", "N genes with related analysable transcript in the matrix", "N genes with related transcript in the matrix after filter for differential missingness", "N genes encompassing at least one CNV among analysable transcripts", "N analysable transcripts encompassing at least one CNV", "min of N genes affected per sample", "max of N genes affected per sample", "min of N genes being NA per sample", "max of N genes being NA per sample", "OR EOAD vs CTRL", "OR EOAD vs CTRL - CIinf", "OR EOAD vs CTRL - CIsup", "OR LOAD vs CTRL", "OR LOAD vs CTRL - CIinf", "OR LOAD vs CTRL - CIsup", "OR clm", "OR clm - CIinf", "OR clm - CIsup", "OR polr", "OR polr - CIinf", "OR polr - CIsup", "p-value (ordinal regression, using clm)", "p-value (ordinal regression, using polr)", "warning clm")
      ...
      dat <- cbind(genematrixAPOE4[, c("sample", "Status", "apoe_genotype", paste0("PC", 1:10))], sum.listGene)
      dat$Status.f <- factor(dat$Status, levels = c("Control", "LOAD", "EOAD"))
      dat$status.listGene <- factor(ifelse(dat$sum.listGene > 0, 1, 0), levels = c(0, 1))
      ...
      if (sum(dat.EOAD$status.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.EOAD$Y <- ifelse(dat.EOAD$Status.f == "EOAD", 1, 0)
        m <- glm(Y ~ status.listGene + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10+ APOE4, data = dat.EOAD, family = binomial(link = "logit"))
        OR.EOAD <- c(exp(m$coef)["status.listGene1"], exp(confint(m))["status.listGene1",])
      }
      ...
      if (sum(dat.LOAD$status.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.LOAD$Y <- ifelse(dat.LOAD$Status.f == "LOAD", 1, 0)
        m <- glm(Y ~ status.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + APOE4, data = dat.LOAD, family = binomial(link = "logit"))
        OR.LOAD <- c(exp(m$coef)["status.listGene1"], exp(confint(m))["status.listGene1",])
      }
      
      # Ordinal regression with clm
      rego <- clm(Status.f ~ status.listGene + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10+ APOE4, data = na.omit(dat))
      confint.rego <- try(exp(confint(rego))["status.listGene1",], silent = TRUE)
      if (inherits(confint.rego, "try-error")) {
        OR.clm <- c(exp(rego$coefficients["status.listGene1"]), NA, NA)
      } else {
      ...
      # Ordinal regression with polr
      rego2 <- polr(Status.f ~ status.listGene + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10+ APOE4, data = na.omit(dat), Hess = TRUE)
      confint.rego2 <- try(exp(confint(rego2))["status.listGene1",], silent = TRUE)
      if (inherits(confint.rego2, "try-error")) {
        OR.polr <- c(exp(rego2$coefficients["status.listGene1"]), NA, NA)
      } else {
      ...
      rego0 <- polr(Status.f ~ 1+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + APOE4, data = na.omit(dat))
      statRV <- -2*(logLik(rego0) - logLik(rego2))
      pvalue.polr <- 1 - pchisq(statRV, df = length(coef(rego2)) - length(coef(rego0)))
      ...
      colnames(res.binary.withAdjustment) <- c("list.of.gene.name", "Adjustment", "N CTRL", "N LOAD", "N EOAD", "N total", "Ncarriers CTRL", "Ncarriers LOAD", "Ncarriers EOAD", "Ncarriers total", "Rate CTRL", "Rate LOAD", "Rate EOAD", "Rate total", "N genes in the list", "N genes with related transcript in the matrix", "N genes with related analysable transcript in the matrix", "N genes with related transcript in the matrix after filter for differential missingness", "N genes encompassing at least one CNV among analysable transcripts", "N analysable transcripts encompassing at least one CNV", "min of N genes affected per sample", "max of N genes affected per sample", "min of N genes being NA per sample", "max of N genes being NA per sample", "OR EOAD vs CTRL", "OR EOAD vs CTRL - CIinf", "OR EOAD vs CTRL - CIsup", "OR LOAD vs CTRL", "OR LOAD vs CTRL - CIinf", "OR LOAD vs CTRL - CIsup", "OR clm", "OR clm - CIinf", "OR clm - CIsup", "OR polr", "OR polr - CIinf", "OR polr - CIsup", "p-value (ordinal regression, using clm)", "p-value (ordinal regression, using polr)", "warning clm")
      ...
      if (sum(dat.EOAD$sum.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.EOAD$Y <- ifelse(dat.EOAD$Status.f == "EOAD", 1, 0)
        m <- glm(Y ~ sum.listGene + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10+ APOE4, data = dat.EOAD, family = binomial(link = "logit"))
        OR.EOAD <- c(exp(m$coef)["sum.listGene"], exp(confint(m))["sum.listGene",])
      }
      ...
      if (sum(dat.LOAD$sum.listGene == 1, na.rm = TRUE) == 0) {
        ...
        dat.LOAD$Y <- ifelse(dat.LOAD$Status.f == "LOAD", 1, 0)
        m <- glm(Y ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + APOE4, data = dat.LOAD, family = binomial(link = "logit"))
        OR.LOAD <- c(exp(m$coef)["sum.listGene"], exp(confint(m))["sum.listGene",])
      }
      
      # Ordinal regression with clm
      rego <- clm(Status.f ~ sum.listGene+ PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + APOE4, data = na.omit(dat))
      confint.rego <- try(exp(confint(rego))["sum.listGene",], silent = TRUE)
      if (inherits(confint.rego, "try-error")) {
        OR.clm <- c(exp(rego$coefficients["sum.listGene"]), NA, NA)
      } else {
      ...
      # Ordinal regression with polr
      rego2 <- polr(Status.f ~ sum.listGene + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + APOE4, data = na.omit(dat), Hess = TRUE)
      confint.rego2 <- try(exp(confint(rego2))["sum.listGene",], silent = TRUE)
      if (inherits(confint.rego2, "try-error")) {
        OR.polr <- c(exp(rego2$coefficients["sum.listGene"]), NA, NA)
      } else {
      ...
      colnames(res.sum.withAdjustment) <- c("list.of.gene.name", "Adjustment", "N CTRL", "N LOAD", "N EOAD", "N total", "Ncarriers CTRL", "Ncarriers LOAD", "Ncarriers EOAD", "Ncarriers total", "Rate CTRL", "Rate LOAD", "Rate EOAD", "Rate total", "N genes in the list", "N genes with related transcript in the matrix", "N genes with related analysable transcript in the matrix", "N genes with related transcript in the matrix after filter for differential missingness", "N genes encompassing at least one CNV among analysable transcripts", "N analysable transcripts encompassing at least one CNV", "min of N genes affected per sample", "max of N genes affected per sample", "min of N genes being NA per sample", "max of N genes being NA per sample", "OR EOAD vs CTRL", "OR EOAD vs CTRL - CIinf", "OR EOAD vs CTRL - CIsup", "OR LOAD vs CTRL", "OR LOAD vs CTRL - CIinf", "OR LOAD vs CTRL - CIsup", "OR clm", "OR clm - CIinf", "OR clm - CIsup", "OR polr", "OR polr - CIinf", "OR polr - CIsup", "p-value (ordinal regression, using clm)", "p-value (ordinal regression, using polr)", "warning clm")
  ...
  # save results
  write.table(resultsB, paste0("RESULTS/Analysis_by_list_of_gene_", list_of_genes, "_PC_ordinal_regression_and_subset_analyses_", type.CNV, "_filtre_freq_", filtre.freq.value, "_recouvrement_", filtre.freq.type.recouvrement, "_differential_missingness_threshold_", p.value_threshold, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE)
