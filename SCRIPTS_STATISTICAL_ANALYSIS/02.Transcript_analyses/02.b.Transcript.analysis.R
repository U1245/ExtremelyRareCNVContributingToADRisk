...
transcript.analysis <- function(type.CNV, filtre.freq.value, filtre.freq.type.recouvrement) {
  ...
  if (type.CNV == "DUPpartial") {
    genematrix <- Matrices$DupPartialMatrice[, c("sample", "Status", "apoe_genotype", transcript.list)]
  }
  ...
  # save results
  # write.table(temp, paste0("RESULTS/Analysis_by_transcript_ordinal_regression_and_subset_analyses_", type.CNV, "_filtre_freq_", filtre.freq.value, "_recouvrement_", filtre.freq.type.recouvrement, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE)
  write.table(temp, paste0("RESULTS/Table_papier_20octobre_Analysis_by_transcript_ordinal_regression_and_subset_analyses_", type.CNV, "_filtre_freq_", filtre.freq.value, "_recouvrement_", filtre.freq.type.recouvrement, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE)
