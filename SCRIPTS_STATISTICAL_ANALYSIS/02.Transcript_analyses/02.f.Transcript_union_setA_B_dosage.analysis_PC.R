...
dosage.analysis_PC <- function(type.CNV, filtre.freq.value, filtre.freq.type.recouvrement) {
  ...
  if (type.CNV == "DEL_DUP") {
    ...
    genematrix1 <- Matrices$DosageMatrice[, c("sample", "Status", "apoe_genotype", transcript.list)]
  }
  ...
  if (type.CNV == "DEL_and_DUPpartial_DUP") {
    ...
    genematrix1 <- Matrices$DosageMatriceWithPartial[, c("sample", "Status", "apoe_genotype", transcript.list)]
  }
  ...
  resultsB <- foreach(iter = 1:B, .combine = "rbind", .packages = c("logistf", "brglm2")) %dopar% {
    ...
    if (dim(table(dat$cnv, dat$Status.f))[1] == 0) {
      ...
      if (0 %in% count["Sum",] | sum(cnvDEL.N)==0 | sum(cnvDUP.N)==0) {
        res <- c(cnv.N, cnvDEL.N, cnvDUP.N, cnv.rate, cnvDEL.rate, cnvDUP.rate, rep(NA, 33))
      } else {
        # cat(paste("AD", iter, "\n"))
        # OR AD
        dat.AD <- dat
  ...
  # save results
  write.table(temp, paste0("RESULTS/Analysis_by_transcript_union_setA_B_PC_adjusted_ordinal_regression_and_subset_analyses_DOSAGE_", type.CNV, "_filtre_freq_", filtre.freq.value, "_recouvrement_", filtre.freq.type.recouvrement, ".txt"), sep = "\t", col.names = TRUE, row.names = FALSE)
